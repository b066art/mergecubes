...
namespace Framework.Utils
{
    public class SwipeDetector : MonoBehaviour
    {
        ...
        {
            //Check if  swipe
            if (verticalMove() > SWIPE_THRESHOLD && verticalMove() > horizontalValMove() || horizontalValMove() > SWIPE_THRESHOLD && horizontalValMove() > verticalMove())
            {
                float deltaX = yDelta();
                float deltaY = xDelta();

                if (deltaX > 0) // up swipe
                {
                    if (deltaY == 0)
                    {
                        OnSwipeUp();
                ...
                else if (deltaX == 0) // left or right swipe
                {
                    if (deltaY > 0)
                    {
                        OnSwipeRight();
